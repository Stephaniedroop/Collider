...
all <- data.frame(matrix(ncol=24, nrow = 0))
...
for (i in 1:length(poss_params)) { 
  ...
  alls <- data.frame(matrix(ncol = 24, nrow = 0))
  for (s in 1:length(s_vals)) {
    allt <- data.frame(matrix(ncol = 24, nrow = 0))
    for (t in 1:length(sens_vals)) {
      mp1 <- data.frame(matrix(ncol = 24, nrow = 0))
      ...
      for (m in 1:modelruns) {
        mpd <- get_cfs(params = poss_params[[i]], structure = 'disjunctive', df = newdfd, s = s_vals[s], sens = sens_vals[t]) # 16 obs of 6
        mpd$run <- m
        mpc <- get_cfs(params = poss_params[[i]], structure = 'conjunctive', df = newdfc, s = s_vals[s], sens = sens_vals[t])
        mpc$run <- m
        mp1 <- rbind(mp1, mpd, mpc)
      }
      allt <- rbind(allt, mp1)
     }
     alls <- rbind(alls, allt)
     #mp1c$pgroup <- i
  ...
  alls$pgroup <- i
  all <- rbind(all, alls) # currently 322560 of 23
} 
