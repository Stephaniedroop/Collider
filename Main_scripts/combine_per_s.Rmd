...
modelNorm <- mp %>%  # 384
  group_by(pgroup, trialtype, structure, node3, uAuB, cond, .drop = FALSE) %>% # it brings 4 back in
  summarise(meancesm = mean(cp)) %>%  # take the mean of the 10 model runs
  mutate(intermed = meancesm*cond) %>% 
  group_by(pgroup, trialtype, structure, node3) %>% 
...
modelPlaceholder <- mp %>% 
  ...
  na.omit() %>% 
  group_by(pgroup, trialtype, structure, node3, realLat, .drop = FALSE) %>% 
  tally
